...
class TestSystemInitialization:
    ...
    def test_system_init_with_valid_config(self, mock_ai_generator, mock_vector_store, mock_config):
...
class TestDataLoadingIssues:
    ...
    def test_empty_vector_store_behavior(self, mock_config, mock_empty_vector_store):
        ...
        with patch('rag_system.VectorStore') as mock_vs, \
            ...
            mock_ai_gen_instance.generate_response.return_value = "I don't have any course information available."
...
class TestQueryExecution:
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.AIGenerator')
    def test_query_with_tool_execution_success(self, mock_ai_generator, mock_vector_store, mock_config, mock_anthropic_tool_response, mock_anthropic_final_response):
        ...
        mock_ai_gen_instance = Mock()
        mock_ai_gen_instance.generate_response.return_value = "MCP enables secure connections to external data."
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.AIGenerator')
    def test_query_with_authentication_error(self, mock_ai_generator, mock_vector_store, mock_config):
        ...
        mock_ai_gen_instance = Mock()
        mock_ai_gen_instance.generate_response.side_effect = Exception("Authentication failed: Invalid API key")
        mock_ai_generator.return_value = mock_ai_gen_instance
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.AIGenerator')
    def test_query_with_invalid_model_error(self, mock_ai_generator, mock_vector_store, mock_config):
        ...
        mock_ai_gen_instance = Mock()
        mock_ai_gen_instance.generate_response.side_effect = Exception("Invalid model: 1claude-sonnet-4-20250514")
        mock_ai_generator.return_value = mock_ai_gen_instance
...
class TestRealWorldScenarios:
    ...
    @patch('rag_system.VectorStore')
    @patch('rag_system.AIGenerator')
    def test_content_search_query(self, mock_ai_generator, mock_vector_store, mock_config):
        """Test a typical content search query"""
        mock_vs_instance = Mock()
        mock_vector_store.return_value = mock_vs_instance
        
        mock_ai_gen_instance = Mock()
        mock_ai_gen_instance.generate_response.return_value = "Based on the course materials, MCP stands for Model Context Protocol."
        mock_ai_generator.return_value = mock_ai_gen_instance
...
class TestErrorDiagnostics:
    ...
    def test_current_config_issues(self):
        ...
        if not os.path.exists(os.path.dirname(config.CHROMA_PATH)):
            issues.append(f"ChromaDB directory does not exist: {os.path.dirname(config.CHROMA_PATH)}")
        ...
        assert any("Invalid model name" in issue for issue in issues), f"Expected to find invalid model name issue in: {issues}"
    
    @patch('rag_system.VectorStore')
    @patch('rag_system.AIGenerator')
    def test_diagnose_query_failure_chain(self, mock_ai_generator, mock_vector_store, mock_config):
        ...
        assert len(failure_points) >= 2  # Should have at least system init and tool registration