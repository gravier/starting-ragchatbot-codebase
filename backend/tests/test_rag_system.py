...
class TestRAGSystem:
    ...
    def test_init(self, mock_session_manager, mock_ai_generator, mock_vector_store, mock_doc_processor, mock_config):
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_tool_registration(self, mock_session_manager, mock_ai_generator, mock_vector_store, mock_doc_processor, mock_config):
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_query_without_session(self, mock_session_manager, mock_ai_generator, mock_vector_store, mock_doc_processor, mock_config):
        """Test query processing without session ID"""
        # Setup mocks
        mock_ai_generator_instance = Mock()
        mock_ai_generator_instance.generate_response.return_value = "Test response about MCP"
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_query_with_session(self, mock_session_manager, mock_ai_generator, mock_vector_store, mock_doc_processor, mock_config):
        """Test query processing with session ID"""
        # Setup mocks
        mock_ai_generator_instance = Mock()
        mock_ai_generator_instance.generate_response.return_value = "Test response with history"
        mock_ai_generator.return_value = mock_ai_generator_instance
        
        mock_session_manager_instance = Mock()
        mock_session_manager.return_value = mock_session_manager_instance
        mock_session_manager_instance.get_conversation_history.return_value = "Previous conversation history"
        ...
        rag_system.tool_manager = Mock()
        rag_system.tool_manager.get_tool_definitions.return_value = []
        rag_system.tool_manager.get_last_sources.return_value = [{"text": "Test Source", "url": None}]
        rag_system.tool_manager.reset_sources.return_value = None
        ...
        mock_session_manager_instance.get_conversation_history.assert_called_with("test_session")
        mock_session_manager_instance.add_exchange.assert_called_with("test_session", "What is MCP?", "Test response with history")
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_query_with_tool_usage(self, mock_session_manager, mock_ai_generator, mock_vector_store, mock_doc_processor, mock_config, tool_manager):
        """Test query processing that uses tools"""
        # Setup mocks
        mock_ai_generator_instance = Mock()
        mock_ai_generator_instance.generate_response.return_value = "MCP enables secure connections to external data sources"
        mock_ai_generator.return_value = mock_ai_generator_instance
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_get_course_analytics(self, mock_session_manager, mock_ai_generator, mock_vector_store, mock_doc_processor, mock_config):
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_add_course_document(self, mock_session_manager, mock_ai_generator, mock_vector_store, mock_doc_processor, mock_config, sample_course, sample_course_chunks):
        """Test adding a single course document"""
        mock_doc_processor_instance = Mock()
        mock_doc_processor_instance.process_course_document.return_value = (sample_course, sample_course_chunks)
...
class TestRAGSystemErrorHandling:
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_ai_generator_error(self, mock_session_manager, mock_ai_generator, mock_vector_store, mock_doc_processor, mock_config):
        """Test handling of AI generator errors"""
        mock_ai_generator_instance = Mock()
        mock_ai_generator_instance.generate_response.side_effect = Exception("API Error: Authentication failed")
        mock_ai_generator.return_value = mock_ai_generator_instance
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_vector_store_error(self, mock_session_manager, mock_ai_generator, mock_vector_store, mock_doc_processor, mock_config):
        """Test handling of vector store errors"""
        mock_vector_store_instance = Mock()
        mock_vector_store_instance.get_course_count.side_effect = Exception("Database connection error")
        mock_vector_store.return_value = mock_vector_store_instance
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_tool_manager_error(self, mock_session_manager, mock_ai_generator, mock_vector_store, mock_doc_processor, mock_config):
        """Test handling of tool manager errors"""
        mock_ai_generator_instance = Mock()
        mock_ai_generator_instance.generate_response.return_value = "Response despite tool error"
        mock_ai_generator.return_value = mock_ai_generator_instance
        ...
        rag_system.tool_manager = Mock()
        rag_system.tool_manager.get_tool_definitions.side_effect = Exception("Tool registration error")
    ...
    @patch('rag_system.AIGenerator')
    @patch('rag_system.SessionManager')
    def test_document_processing_error(self, mock_session_manager, mock_ai_generator, mock_vector_store, mock_doc_processor, mock_config):
        """Test handling of document processing errors"""
        mock_doc_processor_instance = Mock()
        mock_doc_processor_instance.process_course_document.side_effect = Exception("Document parsing error")
        mock_doc_processor.return_value = mock_doc_processor_instance
...
class TestRAGSystemIntegration:
    ...
    def test_prompt_construction(self, mock_config):
        ...
        with patch('rag_system.DocumentProcessor'), \
            ...
            assert "Answer this question about course materials: What is MCP?" in query_prompt
    ...
    def test_source_management(self, mock_config):
        ...
        with patch('rag_system.DocumentProcessor'), \
            ...
            rag_system.tool_manager = Mock()
            rag_system.tool_manager.get_tool_definitions.return_value = []
            rag_system.tool_manager.get_last_sources.return_value = [{"text": "Test Source", "url": "http://example.com"}]
            rag_system.tool_manager.reset_sources.return_value = None
